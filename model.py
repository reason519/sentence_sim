    ...
    def _model(self):
        with tf.variable_scope('emb'):
          ...
          #      self.word_embeddings = tf.get_variable('word_embeddings',initializer=self.init_emb, trainable=False)
                ...
                self.word_embeddings=tf.get_variable('word_embeddings',self.init_emb.shape,initializer=tf.contrib.layers.xavier_initializer(),trainable=False,dtype=tf.float32)
