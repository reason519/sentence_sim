...
def get_sense_ass(word_to_senses_path,sense_to_vector_path):
    word_to_senses = {}
    with open(word_to_senses_path, 'r', encoding='utf-8') as f:
        for line in f:
            word_sen = line.split(' ')
            word_to_senses[word_sen[0].strip()] = [x.strip() for x in word_sen[1:] if x != '']
    sense_to_vector = {}
    with open(sense_to_vector_path, 'r', encoding='utf-8') as f:
        for line in f:
            tokens = line.split()
            vec = np.array(tokens[1:], dtype=np.float32)
            sense_to_vector[tokens[0]] = vec

    ass_vec={}
    for word in word_to_senses:
        count=0.0
        w_vec=np.zeros([300],dtype=np.float32)
        for sense in word_to_senses[word]:
            if sense in sense_to_vector:
                w_vec=np.add(w_vec,sense_to_vector[sense])
            count+=1.0
        np.divide(w_vec,count)
        ass_vec[word]=w_vec
    return ass_vec
